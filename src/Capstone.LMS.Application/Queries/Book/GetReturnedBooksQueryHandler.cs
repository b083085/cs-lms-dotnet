...
{
    public sealed class GetReturnedBooksQueryHandler(
        IBorrowedBookRepository borrowedBookRepository,
        ILogger<GetReturnedBooksQueryHandler> logger) : IRequestHandler<GetReturnedBooksQuery, ListResponseDto<GetBookBorrowedResponseDto>>
    {
        private readonly IBorrowedBookRepository _borrowedBookRepository = borrowedBookRepository;
        private readonly ILogger<GetReturnedBooksQueryHandler> _logger = logger;

        public async Task<ListResponseDto<GetBookBorrowedResponseDto>> Handle(GetReturnedBooksQuery request, CancellationToken cancellationToken)
        {
            // get queryable
            ...
            query = query.Where(p => p.Status == Domain.Enums.BorrowedStatus.Returned);
