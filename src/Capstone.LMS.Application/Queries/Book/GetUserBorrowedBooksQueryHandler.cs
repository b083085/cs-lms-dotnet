...
{
    public sealed class GetUserBorrowedBooksQueryHandler(
        IBorrowedBookRepository borrowedBookRepository,
        ILogger<GetUserBorrowedBooksQueryHandler> logger) : IRequestHandler<GetUserBorrowedBooksQuery, ListResponseDto<GetBookBorrowedResponseDto>>
    {
        private readonly IBorrowedBookRepository _borrowedBookRepository = borrowedBookRepository;
        private readonly ILogger<GetUserBorrowedBooksQueryHandler> _logger = logger;

        public async Task<ListResponseDto<GetBookBorrowedResponseDto>> Handle(GetUserBorrowedBooksQuery request, CancellationToken cancellationToken)
        {
            // get queryable
            ...
            query = query.Where(p => p.UserId == request.UserId && p.Status == Domain.Enums.BorrowedStatus.Borrowed);
