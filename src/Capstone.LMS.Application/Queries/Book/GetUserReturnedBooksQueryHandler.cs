...
{
    public sealed class GetUserReturnedBooksQueryHandler(
        IBorrowedBookRepository borrowedBookRepository,
        ILogger<GetUserReturnedBooksQueryHandler> logger) : IRequestHandler<GetUserReturnedBooksQuery, ListResponseDto<GetBookBorrowedResponseDto>>
    {
        private readonly IBorrowedBookRepository _borrowedBookRepository = borrowedBookRepository;
        private readonly ILogger<GetUserReturnedBooksQueryHandler> _logger = logger;

        public async Task<ListResponseDto<GetBookBorrowedResponseDto>> Handle(GetUserReturnedBooksQuery request, CancellationToken cancellationToken)
        {
            // get queryable
            ...
            query = query.Where(p => p.UserId == request.UserId && p.Status == Domain.Enums.BorrowedStatus.Returned);
